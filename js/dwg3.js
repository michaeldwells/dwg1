...
let tiles;
let panic = 20;
do {panic -= 1;tiles = all_tris.map(x=>popt(x));} while (panic > 0 && new Set([...tiles,...tiles.map(reverse)]).size < tiles.length*2);
LOG(all_words);
//let game_words = all_words.map(x=>opt(x));
//let tiles = game_words.map((x,i)=>x.slice(0,x.indexOf(all[i]))+x.slice(x.indexOf(all[i])+3));
tiles = tiles.map(x=>popt([0,1])==0 ? x : reverse(x));
